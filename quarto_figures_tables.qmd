# Tables & Figures in Quarto {#sec-appendix}
...
dem %>% 
  ...
  kable_styling(bootstrap_options = "none", #remove all horizontal lines
                font_size = 12,
                full_width = TRUE) %>% 
  row_spec(row = 0, align = "c", italic = TRUE) %>% 
  pack_rows("Sex", 2, 3, bold = FALSE, label_row_css = "border-bottom: none;") %>%  #remove horizontal line below packed rows
  pack_rows("Race", 4, 8, bold = FALSE, label_row_css = "border-bottom: none;") %>% 
  pack_rows("Hispanic, Latino, or Spanish Origin", 9, 10, bold = FALSE, label_row_css = "border-bottom: none;") %>% 
  pack_rows("Education", 11, 16, bold = FALSE, label_row_css = "border-bottom: none;") %>% 
  pack_rows("Employment", 17, 25, bold = FALSE, label_row_css = "border-bottom: none;") %>% 
  pack_rows("Marital Status", 27, 31, bold = FALSE, label_row_css = "border-bottom: none;") %>% 
  footnote(general=footnote_table_dem, threeparttable = TRUE) %>%
  column_spec(1, width = "4in") %>% #specify column width
  column_spec(2:6, width = "1in") %>% #specify column width
  row_spec(0, extra_css = "border-bottom: 1px solid; border-top: 1px solid") %>% #add borders on the header row
  row_spec(nrow(dem), extra_css = "border-bottom: 1px solid") #add a border after the last row, before the footnote
```
