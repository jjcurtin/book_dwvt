...
# use a for loop with indexed colors
# for even more participants, you could also use `RColorBrewer::brewer.pal()` or `rainbow()` to automatically generate colors.
participants <- unique(gps_participants$participant)
colors <- c("#C5050C", "#2E86AB")  # red, blue

# add points for each participant as separate groups
for (i in seq_along(participants)) {
  participant_data <- gps_participants |> filter(participant == participants[i])
  ...
  map <- map |>
    addCircleMarkers(
      ...
      radius = 6,
      color = "#000",
      fillColor = colors[i],  # index into color vector
      fillOpacity = 0.7,
      weight = 1,
      popup = ~paste0(
        "<strong>", name, "</strong><br>",
      ...
      group = participants[i]  # use indexed participant name
    )
}
