...
`foreach()` in parallel mode using `%dopar%` but without a parallel plan.  in contrast to `future_map()`, no plan is needed for `foreach()`.  You should use it without a plan!
```{r}
tic()
